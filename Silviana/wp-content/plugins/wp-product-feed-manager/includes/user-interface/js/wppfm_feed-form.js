/**
 * Contains the functions for the Feed Form.
 * 
 * This file contains javascript functions required to generate the wp-product-feed-manager-add-new-feed page.
 * 
 * @link	wp-content/plugins/wp-product-feed-manager/includes/user-interface/js/wppfm_feed-form.js
 * @file	This file contains javascripts
...
function wppfm_getCategoryChildren( parentCategoryId ) {
	return $jq( '#feed-selector-' + parentCategoryId ).attr( "data-children" ) ? JSON.parse( $jq( '#feed-selector-' + parentCategoryId ).attr( "data-children" ) ) : [ ];
}
...
function wppfm_setChildrenToParentCategory( parentId, childId, level, selectedCategory ) {
	...
	for( var i=0; i < level; i++ ) {
		curCatStr += $jq( '#catmap-' + parentId + '_' + i ).val() + ' > ';
	}

	var preSelectionParentCategory = curCatStr.substring( 0, curCatStr.length - 3 );
	
	if ( currentChildCategory === preSelectionParentCategory || currentChildCategory === 'Map to Default Category' ) {
		if ( '#category-text-span-' + childId.length === 0 ) {
			wppfm_mapToDefaultCategoryElement( childId, newCategory );
		}
		
		$jq( '#category-text-span-' + childId ).text( newCategory );
		...
		_feedHolder.mapCategory( categorySelectorId, selectedCategory );
	} else if ( $jq( '#category-text-span-' + childId ).length === 0 ) {
		$jq( '#feed-category-' + childId ).html( wppfm_mapToDefaultCategoryElement( childId, currentChildCategory ) );
		$jq( '#category-selector-catmap-' + childId ).hide();
	}
}

function wppfm_getCategorySelectorValue( selectorId ) {
	var val = $jq( '#category-text-span-' + selectorId ).text();
	
	if ( val === '' ) {
		var catString = '';
		
		for ( var i = 0; i < 7; i++ ) {
			var cat = $jq( '#catmap-' + selectorId + '_' + i ).val();
			
			if ( cat !== null && cat !== '0' )
				catString += $jq( '#catmap-' + selectorId + '_' + i ).val() + ' > ';
		}
...
function wppfm_editCategoryMapping( id ) {
	...
	var channel = _feedHolder['channel'];
	var language = wppfm_channelCountryCode( channel );
	var currentCategoryString = $jq( '#category-text-span-' + id ).text();
	var currentCategoryArray = currentCategoryString.split( ' > ' );
	
	if ( currentCategoryString && currentCategoryString !== 'Map to Default Category' ) {
...
function wppfm_childrensValue( parentId ) {
	...
	for( var i = 0; i < childrenIds.length; i++ ) {
		...
		if( childsValue !== '0' && childsValue !== 'Map to Default Category' ) { 
			result = childsValue;
			$jq( '#feed-category-' + childrenIds[i] ).html( wppfm_mapToDefaultCategoryElement( childrenIds[i], result ) );
			$jq( '#category-selector-catmap-' + childrenIds[i] ).hide();
		} else {
...
function wppfm_showCategoryMappingSelectors( id, catList, currentCategoryArray ) {
	for( var i = 0; i < catList.length; i++ ) {
		if ( catList[i] && catList[i].length > 0 ) {
			...
			$jq( '#catmap-' + id + '_' + i ).append( wppfm_categorySelectCntrl( catList[i] ) );
			$jq( '#catmap-' + id + '_' + i ).val( selectedCategory ); 
			$jq( '#catmap-' + id + '_' + i ).show();
		} 

		$jq( '#catmap-' + id + '_' + i ).prop( 'disabled', false );
	}

	$jq( '#feed-category-' + id ).html( "" ); // remove the category string so it can be replaced by the selector
	$jq( '#category-selector-catmap-' + id ).show(); // now show the category selectors
}
...
function wppfm_activateOptionalFieldRow( level, name ) {
	...
	$jq( "#output-field-cntrl-" + level ).val( 'no-value' );
...
function wppfm_getCombinedSeparatorList( selectedValue ) {
	...
	var separatorOptions = { "0": "-- No separator --", "1": "space", "2": "comma", "3": "point", "4": "semicolon", "5": "colon", "6": "dash", "7": "slash", "8": "backslash", "9": "double pipe" };
